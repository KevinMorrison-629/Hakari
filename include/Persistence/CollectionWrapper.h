...
public:
    ...
    std::vector<T> GetAll(const std::unordered_map<std::string, FieldValue> &fields, size_t max_count = 999)
    {
        ...
        auto cursor = m_Collection.aggregate(pipeline, mongocxx::options::aggregate{}.limit(max_count));
                     ...
                     const std::unordered_map<std::string, FieldValue> &fields)
    {
        try
        {
            ...
            {
                filter_doc.append(MakeDocument(key, value));
            }
            ...
            for (const auto &[key, value] : fields)
            {
                set_doc.append(MakeDocument(key, value));
            }
                     ...
                     const FieldValue &newElement)
    {
        try
        {
            ...
            push_doc.append(MakeDocument(arrayField, newElement));
                       ...
                       const FieldValue &valueToRemove)
    {
        try
        {
            ...
            pull_doc.append(MakeDocument(arrayField, valueToRemove));
