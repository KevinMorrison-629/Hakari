...
namespace Core::Net
{
    ...
    /// Configures the listen socket with the global connection status callback and sets this ServerManager
            ...
            if (numMsgs > 0)
            {
                for (int i = 0; i < numMsgs; ++i)
                {
                    if (pIncomingMsgs[i] && pIncomingMsgs[i]->m_cbSize > 0)
                    {
                        ...
                        // if (OnClientMessageReceived) { OnClientMessageReceived(hConn, msg); }
