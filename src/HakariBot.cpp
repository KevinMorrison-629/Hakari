...
HakariBot::HakariBot(dpp::cluster *botRef, const std::map<std::string, uint64_t> &arguments)
{
    ...
    if (arguments.find("--logActiveGuilds") != arguments.end())
    {
        ...
        {
            std::string entry = guild.first.str() + " - " + guild.second.name +
                                " [members: " + std::to_string(guild.second.member_count) + "]";
    ...
    m_bot->on_slashcommand(
        [this](const dpp::slashcommand_t &command_event)
        {
    ...
    m_bot->on_message_reaction_add(
        [this](const dpp::message_reaction_add_t &reaction_event)
        {
...
HakariBot::~HakariBot() { Utils::Logger::Log(Utils::Logger::Type::INFO, "DiscordBot Shutting Down. . ."); }
...
void HakariBot::RegisterCommands(const dpp::snowflake &guild_id)
{
    ...
    Utils::Logger::Log(Utils::Logger::Type::INFO,
                       "Registered " + std::to_string(numRegisteredCommands) + " Discord Commands.");
}
